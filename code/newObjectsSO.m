function [tracks,table,wAssoc] = newObjectsSO(tracks,table,wAssoc,PPP,W,gating_matrix, ...
    lik_matrix,used_meas_u_not_d,k,model,filter)

indices = 1:size(W,2);

W = W(:,used_meas_u_not_d);
gating_matrix = gating_matrix(used_meas_u_not_d,:);
lik_matrix = lik_matrix(used_meas_u_not_d,:);
...
for i = 1:m
    meas_cell{i} = i;
    Lc(i) = likPPP(W(:,i),lik_matrix(i,:),PPP,gating_matrix(i,:),model,filter);
end
    ...
    if length(meas_cell{cell_idx})==1 %selected cell has only one measurement
        ...
        for i = 1:N
            ...
            elseif any(sum(gating_matrix(meas_cell{i},:),1)>=1 & gating_matrix(mea_selected,:))
                ...
                if any(in_gate)
                    L_move1(i) = likPPP(W(:,[meas_cell{i} mea_selected]),lik_matrix([meas_cell{i} mea_selected],:),PPP,in_gate,model,filter);
        ...
        for i = 1:N
            ...
            elseif any(sum(gating_matrix(meas_cell{i},:),1)>=1 & gating_matrix(mea_selected,:))
                if length(temp) == 1
                    ...
                    if any(in_gate)
                        L_move2 = likPPP(W(:,temp),lik_matrix(temp,:),PPP,in_gate,model,filter);
                    else
                        L_move2 = -inf;
                    end
                end
        ...
        if length(temp) == 1
            ...
            in_gate = sum(gating_matrix(temp,:),1)>=1;
            if any(in_gate)
                Lp_move_new_cell = likPPP(W(:,temp),lik_matrix(temp,:),PPP,in_gate,model,filter);
            else
        ...
        for i = 1:N
            if i~=cell_idx
                ...
                if any(in_gate)
                    Lp_merge(i) = likPPP(W(:,[meas_cell{i} meas_cell{cell_idx}]),lik_matrix([meas_cell{i} meas_cell{cell_idx}],:),PPP,in_gate,model,filter);
        ...
        if length(meas_cell{cell_idx}(IDX==1)) == 1
            ...
            if any(in_gate)
                Lp_split1 = likPPP(W(:,meas_cell{cell_idx}(IDX==1)),lik_matrix(meas_cell{cell_idx}(IDX==1),:),PPP,in_gate,model,filter);
            else
                Lp_split1 = -inf;
            end
        end
        if length(meas_cell{cell_idx}(IDX==2)) == 1
            ...
            if any(in_gate)
                Lp_split2 = likPPP(W(:,meas_cell{cell_idx}(IDX==2)),lik_matrix(meas_cell{cell_idx}(IDX==2),:),PPP,in_gate,model,filter);
            else
                Lp_split2 = -inf;
            end
        end
...
for i = 1:n_tt
    ...
    [Bern,lik] = detectionPPP(W(:,bestPartition{i}),PPP,in_gate,model,filter);
    Bern.t_birth = k;
    Bern.t_death = k;
    Bern.w_death = 1;
