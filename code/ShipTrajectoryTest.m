...
%         q % q for cv2 or cv3 model        
        ...
        motionType % String;
        X %Shape matrix encoding elliptic shape
        tbirth %timestep of object birth
        tdeath %timestep of object death
    end
...
%         end
        function obj = ShipTrajectoryTest(extend,groundTruth,tbirth,tdeath)
            obj.extendAngles = extend.angles';
            obj.extendRadii = extend.hull_r;
            extendBody = extend.hull_b;
            obj.extendBody = extendBody;
            Nt = size(groundTruth,2);
            xKin = zeros(6,Nt);
            for i = 1:Nt
                startState = groundTruth{1,i};
                xKin(:,i) = [startState.N; startState.E; deg2rad(startState.heading); startState.velN; startState.velE; deg2rad(startState.angVel)];
            end
            obj.xKin = xKin;
            obj.extendNE = getExtendNE(extendBody, xKin(1:2,:), xKin(3,:)');
            obj.lowerextendNE = extend.hull_b;
        ...
        function obj = adjustTrackLength(obj,range)
            survRange = range+max(obj.extendRadii)+1;
            xC = obj.xKin(1:2,:);
            Nt = size(xC,2);
            tBirth = 0;
            tDeath = Nt;
            exist = false;
            for i = 1:Nt
                range = sqrt(xC(1,i)^2+xC(2,i)^2);
                if(range<survRange && ~exist)
                    tBirth = i;
                    exist = true;
                end
                if(range>survRange && exist)
            ...
            if(tBirth>obj.tbirth)
                obj.tbirth = tBirth;
            end
            if(tDeath<obj.tdeath)
                obj.tdeath = tDeath;
            end
        end
    end
